...
public class Version implements Comparable<Version> {
    ...
    @Nullable
    public static Version parseVersionSafe(String version) {
        try {
    ...
    private void validate() {
        ...
        for (int i = 0; i < length; i++) {
            if ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-".indexOf(qualifier.charAt(i)) ==
                -1) { //$NON-NLS-1$
     ...
     */
    @Override
    public int compareTo(@NotNull Version object) {
        if (object == this) { // quicktest
            return 0;
        }
